                ...
                ${activeDraft.importSource ? `
                    <div class="imported-source-banner">
                        ...
                        <div>
                            ...
                            <span>SHA-256 ${escapeHtml(String(activeDraft.importSource.sha256 || '').slice(0, 16))}Ã¢â‚¬Â¦ Ã‚Â·
...
(function () {
        ...
        return `
            <section class="sl-catalog-panel">
                ...
                <div class="sl-toolbar">
                    <label class="sl-search"><i class="fa-solid fa-magnifying-glass"></i><input id="slUnitSearch" type="search" value="${escapeHtml(state.query)}" placeholder="Cari kode lambung, kategori, atau lokasiÃ¢â‚¬Â¦" data-sl-filter="query"></label>
                    ...
                    <div class="sl-modal-scroll">
                        ...
                        <section class="sl-modal-section sl-spb-builder">
                            ...
                            <div class="sl-draft-actions">
                                ...
                                <label><span>Catatan permintaan</span><input id="spb-notes" type="text" value="${escapeHtml(state.modal.notes || '')}" placeholder="Keterangan, spesifikasi alternatif, atau kendalaÃ¢â‚¬Â¦" data-sl-modal-field="notes"></label>
    ...
    function render() {
        ...
        if (!ensureInitialized()) {
            root.innerHTML = `<div class="sl-loading"><i class="fa-solid fa-database"></i><strong>Menyiapkan registry aset dan Work OrderÃ¢â‚¬Â¦</strong><span>Menunggu data.json selesai dimuat.</span></div>`;
            return;
        }
        root.innerHTML = `
            <div class="sl-shell">
    ...
    function addApprovalRow(request) {
        ...
        row.innerHTML = `
            ...
            <td>SPB Spare Part Ã‚Â· ${escapeHtml(shortCode(request.assetId))}</td>
            <td>Maintenance / Logistik</td>
            <td>${request.lines.length} item Ã‚Â· ${escapeHtml(request.woId || 'Belum terkait WO')}</td>
            <td>
...
(function () {
    ...
    function previewProfile(asset) {
        if (state.units[asset.id]) return state.units[asset.id];
    ...
    function overallStatus(summary) {
        if (summary.critical) return 'danger';
    ...
    function latestConditionFor(assetId) {
        ...
        return state.history.find(item => item.assetId === assetId)
            || state.history.find(item => shortCode(item.assetId) === code)
            || null;
    }
    ...
    function renderCatalog() {
        ...
        const rows = assets.map(asset => {
            ...
            return { asset, profile, summary, overall: overallStatus(summary) };
        });
        const totalCritical = rows.filter(item => item.overall === 'danger').length;
        const totalWarning = rows.filter(item => item.overall === 'warning').length;
        const totalSafe = rows.filter(item => item.overall === 'success').length;
        ...
        const filtered = rows
            .filter(item => {
                ...
                return (!query || haystack.includes(query))
                    && (catalogState.status === 'ALL' || item.overall === catalogState.status)
                    && (catalogState.type === 'ALL' || type === catalogState.type);
            })
            .sort((a, b) => {
                const rank = { danger: 0, warning: 1, success: 2 };
                return rank[a.overall] - rank[b.overall] || shortCode(a.asset.id).localeCompare(shortCode(b.asset.id));
            });
        ...
        return `
            ...
            <section class="cm-catalog-kpis">
                <article class="cm-catalog-kpi info"><div><span>Unit Tertabulasi</span><strong>${rows.length}</strong><small>Satu baris per kode lambung</small></div><i class="fa-solid fa-truck-fast"></i></article>
                <article class="cm-catalog-kpi danger"><div><span>Unit Kondisi Kritis</span><strong>${totalCritical}</strong><small>Perlu validasi / tindak lanjut</small></div><i class="fa-solid fa-triangle-exclamation"></i></article>
                <article class="cm-catalog-kpi warning"><div><span>Perlu Perhatian</span><strong>${totalWarning}</strong><small>Rotasi, load test, atau jadwal</small></div><i class="fa-solid fa-screwdriver-wrench"></i></article>
                <article class="cm-catalog-kpi success"><div><span>Dalam Batas Aman</span><strong>${totalSafe}</strong><small>Monitoring rutin dilanjutkan</small></div><i class="fa-solid fa-shield-check"></i></article>
            </section>
            ...
            <section class="cm-catalog-panel">
                ...
                <div class="cm-catalog-toolbar">
                    <div class="cm-catalog-search"><i class="fa-solid fa-magnifying-glass"></i><input id="cm-catalog-search" type="search" value="${esc(catalogState.query)}" placeholder="Cari kode lambung, kategori, atau lokasiÃ¢â‚¬Â¦" oninput="ConditionMonitoring.filterCatalog(this.value)"></div>
                    ...
                    <select class="form-control" onchange="ConditionMonitoring.setCatalogFilter('type', this.value)">
                        ...
                        <option value="CUTTING" ${catalogState.type === 'CUTTING' ? 'selected' : ''}>Recycler / Cutting Bit</option>
        ...
        function walk(current) {
            ...
            switch (current.localName) {
                ...
                case 'checkBox': {
                    ...
                    parts.push(checked && !['0', 'false', 'off'].includes(value) ? 'Ã¢Ëœâ€™' : 'Ã¢ËœÂ');
                    return;
                }
                case 'instrText':
                    return;
        ...
        if (!selectedSchema) {
            ...
            if (missing.length) {
                qualityWarnings.push({
                    ...
                    message: `${missing.length} field wajib belum ditemukan otomatis: ${missing.slice(0, 8).map(item => item.label).join(', ')}${missing.length > 8 ? ', Ã¢â‚¬Â¦' : ''}.`,
                    severity: 'warning',
...
(function () {
            ...
            <section class="official-template-library" aria-labelledby="officialTemplateTitle">
                ...
                <div id="officialTemplateBody">
                    <div class="official-template-toolbar">
                        <div class="report-control-wrap">
                            <i class="fa-solid fa-magnifying-glass"></i>
                            <input type="search" class="report-input" id="officialTemplateSearch" placeholder="Cari kode atau nama templateÃ¢â‚¬Â¦">
                        </div>
    ...
    function renderOfficialTemplateLibrary() {
        ...
        grid.querySelectorAll('[data-download-official-template]').forEach(button => {
            button.addEventListener('click', async () => {
                ...
                button.disabled = true;
                button.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> MenyiapkanÃ¢â‚¬Â¦';
                try {
    ...
    async function enqueueFiles(files) {
        ...
        for (let index = 0; index < files.length; index += 1) {
            ...
            updateTransient(queueId, {
                status: 'processing',
                progress: { current: 0, total: 1, message: 'Menyiapkan parserÃ¢â‚¬Â¦' }
            });
            try {
    ...
    function updateTransient(importId, values, shouldRender = true) {
        ...
        if (shouldRender) renderQueue();
            ...
            if (card) {
                ...
                if (label) label.textContent = values.progress?.message || 'MemprosesÃ¢â‚¬Â¦';
            }
        }
    }
                    ...
                    <div class="import-queue-copy">
                        ...
                        ${summary.status === 'processing' || summary.status === 'queued' ? `
                            <div class="import-card-progress" role="progressbar" aria-label="Progres ${escapeHtml(summary.fileName)}"
                                ...
                                <small>${escapeHtml(summary.progress?.message || 'MemprosesÃ¢â‚¬Â¦')}</small>
                            </div>
                        ` : ''}
                    </div>
                </button>
    ...
    async function renderDetail() {
        ...
        if (summary.status === 'processing' || summary.status === 'queued') {
            container.innerHTML = `
                <div class="import-detail-empty processing">
                    ...
                    <p>${escapeHtml(summary.progress?.message || 'Dokumen sedang diprosesÃ¢â‚¬Â¦')}</p>
                </div>
            `;
            return;
        }
        ...
        container.innerHTML = `
            <div class="import-detail-loading">
                <i class="fa-solid fa-spinner fa-spin"></i> Memuat hasil ekstraksiÃ¢â‚¬Â¦
            </div>
        `;
            ...
            <header class="import-detail-header">
                <div class="import-detail-title">
                    ...
                    <div>
                        ...
                        <p>${formatBytes(record.source.size)} Ã‚Â· ${escapeHtml(unitSummary)} Ã‚Â· SHA-256 <code>${escapeHtml(record.source.sha256.slice(0, 16))}Ã¢â‚¬Â¦</code></p>
                    </div>
                </div>
    ...
    function manualMappingControl(field, value = '') {
        ...
        if (field.type === 'select') {
            return `<select ${common}>
                <option value="">Pilih ${escapeHtml(field.label.toLowerCase())}Ã¢â‚¬Â¦</option>
    ...
    function contentDetail(record) {
        return `
            <div class="import-content-toolbar">
                ...
                <input type="search" class="report-input" id="importContentSearch" placeholder="Cari isi atau source referenceÃ¢â‚¬Â¦">
            </div>
    ...
    async function saveManualMapping(record, form, button) {
        ...
        if (missing.length) {
            record.quality.warnings.push({
                code: 'required_fields_missing',
                message: `${missing.length} field wajib masih perlu dilengkapi: ${missing.slice(0, 8).map(field => field.label).join(', ')}${missing.length > 8 ? ', Ã¢â‚¬Â¦' : ''}.`,
                severity: 'warning',
                sourceRef: schema.id
            });
        }
    ...
    function bindDeleteButtons(container) {
        container.querySelectorAll('[data-delete-import]').forEach(button => {
            button.addEventListener('click', async () => {
                ...
                const confirmed = window.confirm(
                    `Hapus arsip impor Ã¢â‚¬Å“${summary?.fileName || button.dataset.deleteImport}Ã¢â‚¬Â?\n\n`
    ...
    function applyToDraft(record, button) {
        ...
        if (record.quality.errors) {
            const confirmed = window.confirm(
                `Ekstraksi Ã¢â‚¬Å“${record.source.fileName}Ã¢â‚¬Â memiliki ${record.quality.errors} error audit.\n\n`
        ...
        if (draftState?.hasData) {
            const confirmed = window.confirm(
                `Template ${record.target.code} sudah memiliki draft.\n\nGanti draft tersebut dengan hasil ekstraksi Ã¢â‚¬Å“${record.source.fileName}Ã¢â‚¬Â?`
            );
            if (!confirmed) return;
        }
        try {
    ...
    async function exportRecord(record, button) {
        const original = button.innerHTML;
        button.disabled = true;
        button.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Menyusun JSONÃ¢â‚¬Â¦';
                ...
                <div class="pk-tab-content active" id="tab-head">
                    ...
                    <div class="pk-panel">
                        <div class="pk-panel-header">
                            ...
                            <span class="text-muted" style="font-size:0.85rem;">Formula: Nilai Bobot = Skor Ãƒâ€” Bobot ÃƒÂ· 5</span>
                        </div>
                    ...
                    <div class="prod-cards-grid">
                        <div class="prod-card success">
                            <div class="prod-card-info">
                                ...
                                <div class="prod-sub">Target Benchmark: Ã¢â€°Â¥ 90.0%</div>
                            </div>
                        ...
                        <div class="prod-card success">
                            <div class="prod-card-info">
                                ...
                                <div class="prod-sub">Target Benchmark: Ã¢â€°Â¥ 80.0%</div>
                            </div>
                        ...
                        <div class="prod-card info">
                            <div class="prod-card-info">
                                ...
                                <div class="prod-sub">Target Benchmark: Ã¢â€°Â¤ 10.0%</div>
                            </div>
                    ...
                    <div class="prod-cards-grid">
                        <div class="prod-card primary">
                            <div class="prod-card-info">
                                ...
                                <div class="prod-sub">Target Benchmark: Ã¢â€°Â¥ 100 Jam</div>
                            </div>
